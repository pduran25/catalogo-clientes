...
export default function  DataExtra(props) {
    ...
    const {texto, familia, categoria} = props;
...
useEffect(() => {
        ...
        if(familia != 0){
            if(categoria != 0){
                setSql("SELECT * FROM Producto WHERE pr_referencia LIKE ? AND pr_codfamilia = ? AND pr_codnivel1 = "+categoria)
            }else{
                setSql("SELECT * FROM Producto WHERE pr_referencia LIKE ? AND pr_codfamilia = ? AND pr_codnivel1 != "+categoria)
            }  
        }else{
                setSql("SELECT * FROM Producto WHERE pr_referencia LIKE ? AND pr_codfamilia != ? AND pr_codnivel1 != 0")
        }
    setCont(cont+1)
}, [texto,familia, categoria]);
   ...
   useCallback(() => {
    ...
    return (
        ...
        <View>
            {((posts.length <= 0) || (texto.length == 0 && familia == 0 && categoria == 0)) ? (
                <NoFoundProducts />
            ) : (<View>
                <FlatList
                data = {posts}
